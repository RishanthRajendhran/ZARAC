...
while getopts 'a:b:c:de:f:g:h:jl:m:n:o:p:q:s:t:v:w:x' opt; do
  case "$opt" in
    a)   LOGFILE="$OPTARG"  ;;
    b)   BATCHSIZE="$OPTARG"  ;;
...
$LAUNCHER trainTest.py -episodes $EPISODES -dataPrefix $DATAPPREFIX -trainName $TRAINNAME -testName $TESTNAME -model $MODEL -size $MODELSIZE -dataset $DATASET -maxShots $MAXSHOTS -trainPrompt $TRAINPROMPT -testPrompt $TESTPROMPT -numEpochs $NUMEPOCHS -maxSteps $MAXSTEPS -batchSize $BATCHSIZE -learningRate $LEARNINGRATE -savePath $SAVEPATH -log $LOGFILE -lrScheduler $LEARNINGRATESCHEDULER $ADDITIONAL